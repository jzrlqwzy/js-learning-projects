...
const whereAmI = function (lat, lng) {
  fetch(
    `https://geocode.xyz/${lat},${lng}?geoit=json&auth=12561839940755388207x9328`
  )
    .then(response => {
      if (!response.status)
        throw new Error(`You request too often ${response.status}`);
      return response.json();
    })
    .then(data => {
...
const loadAll = async function (imgArr) {
  try {
    ...
    // Promise.all takes in an array of promises and return back an array of fulfilled
